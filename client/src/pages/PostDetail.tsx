...
import { fetchPostComments, addComment, addNewCommentLocally } from '../store/comment/comment.slice';
...
const PostDetail: React.FC = () => {
    ...
    const buildCommentTree = (flatComments: any[]) => {
        ...
        // Initialize map with comment objects and empty repliesData
        ...
        flatComments.forEach(comment => {
            ...
            if (comment.parentComment) {
                const parent = commentMap.get(typeof comment.parentComment === 'string' ? comment.parentComment : comment.parentComment._id);
                if (parent) {
                    parent.repliesData.push(current);
                } else {
                    // Fallback to roots if parent not found (shouldn't happen with full data)
                    tree.push(current);
                }
            } else {
                tree.push(current);
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white">
            ...
            <main className="max-w-4xl mx-auto px-4 pt-20">
                ...
                <div className="mt-8 bg-[#1a1a1a] rounded-2xl border border-[#333] p-6 mb-10 text-justify">
                    <h3 className="text-xl font-bold mb-8 flex items-center gap-2 text-justify">
                    ...
                    <form onSubmit={handleSubmitComment} className="flex gap-4 mb-10">
                        ...
                        <div className="flex-1 relative">
                            <input
                                ...
                                className="w-full bg-[#2a2a2a] border border-[#333] rounded-2xl py-3 px-6 pr-12 outline-none focus:border-blue-500 transition-all font-medium"
                            />
                            <button
                                type="submit"
